...
The auto-flow edition modes allow to modify the pipes after the edited elements to maintain system connectivity automatically.  The following options are allowed: 'All Branches Downstream', 'All Branches Upstream', 'Only Selection'.
...
Holds the highest point from the edited pipes as the elevation reference and allows adjustment of the slope. The user can modify multiple rows slope at the same time.


### 2.2 Hold Reference: Pipe Lowest Point

Holds the lowest point from the edited pipes as the elevation reference and allows adjustment of the slope. The user can modify multiple rows slope at the same time.


### 2.3 Hold Slope

Holds the slope of the edited pipes. The user can adjust start/end elevations and move the entire system up/down with the difference value.
...
-  **Set Hold Reference and Auto-flow edition modes**. Set the hold reference to the highest point of edited pipes. 
-  **Set auto-flow edition** to "All Branches Downstream".
-  **Set pipe new values**. Enter new slope in multiple pipes (e.g. 3%).
-  **Apply changes**. The system adjusts selected pipes and all downstream pipes.
...
-  **Identify pipes** that need adjustment and select them.
-  **Set Hold Reference and Auto-flow edition modes**. Set the hold reference to the lowest point of edited pipes. 
-  **Set auto-flow edition** to "All Branches Upstream".
-  **Set pipe new values**. Enter new slope in multiple pipes (e.g. 2.5%).
-  **Apply changes**. The system adjusts selected pipes and all upstream pipes.
...
-  **Identify pipes** to modify and select them. In this case the first pipe or pipes of the system (for 'Only Selection' mode) to move up or down.
-  **Set Hold Reference and Auto-flow edition modes**. Set the hold reference to "Hold Slope". Set auto-flow edition to "Only Selection".
